...
const submitData=(data)=>{
   reset();
   toast(<SuccessMessage/>,{hideProgressBar:true,closeButton:false,position:'top-center',autoClose:3000,className:"toast_classname_mobile"})
...
<FormControlLabel  label='General Enquiry' value='general' control={<Radio size='small'  name='query_type' onChange={field.onChange} sx={{'&.Mui-checked': {
  ...
  fontStyle: 'normal'},margin:'-8px'}}/>
    ...
    <Box className='label2_2' sx={{marginTop:"5px", borderRadius: '5px',
 border:'1px solid rgb(202, 201, 201)',
    height: '1.4375em',
    padding: '8.5px 20px',
    display:'block','&:hover':{border:`1px solid ${theme.mode.light.buttonColor.backgroundColor}`}}}>
...
<Controller control={control} name='agreement' rules={{required:'To submit this form, please consent to being contacted'}} render={({field})=>(
    <FormControlLabel label='I hereby constent to being contacted by the team *' sx={{'& .MuiTypography-root':{fontSize:'0.75rem'}}} control={<Checkbox size='small' sx={{'& .MuiSwitchBase-root':{'&:hover':{cursor:'pointer'}},'&.Mui-checked': {
    color: theme.mode.light.buttonColor.backgroundColor
